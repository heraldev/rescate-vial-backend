...
describe('=== Pruebas Unitarias: Auth Controller ===', () => {
  ...
  describe('POST /api/auth/register', () => {
    it('Escenario 1 (Feliz): Debería registrar un usuario correctamente', async () => {
      const mockUser = { id: 1, email: 'test@heraldev.online' };
      authService.registerUser.mockResolvedValue(mockUser);

      const res = await request(app)
        .post('/api/auth/register')
        .send({ name: 'Eduardo', email: 'test@heraldev.online', password: 'securePassword123' });
    ...
    it('Escenario 2 (Error): Debería retornar 400 si el correo ya está duplicado', async () => {
      authService.registerUser.mockRejectedValue(new Error('El correo ya está registrado'));

      const res = await request(app)
        .post('/api/auth/register')
        .send({ email: 'test@heraldev.online' });
  ...
  describe('POST /api/auth/login', () => {
    it('Escenario 1 (Feliz): Debería autenticar correctamente y guardar bitácora SUCCESS', async () => {
      const mockLoginResult = { token: 'mockTokenJWT', user: { id: 1, email: 'user@test.com' } };
      ...
      expect(authModel.registerLoginAttempt).toHaveBeenCalledWith(expect.objectContaining({
        status: 'SUCCESS',
        email: 'user@test.com'
      }));
    });
    ...
    it('Escenario 2 (Error de Credenciales): Debería retornar 400 y registrar bitácora FAILED', async () => {
      authService.loginUser.mockRejectedValue(new Error('Contraseña incorrecta'));
      authModel.registerLoginAttempt.mockResolvedValue();
